...
## Purpose & Scope
- **Goal:** build an autonomous Manifold prediction-market agent that consistently seeks profit while logging all reasoning and keeping risk bounded.
...
2. **Connectors** – Manifold client (read-only → execution) plus future BaseConnector for Polymarket/paper venues.
...
| --- | --- | --- |
| 0 | Core agent interface/types | `core/agent_interface.ts` |
| 1 | Manifold read-only connector | `connectors/manifold.ts` (list/get markets/groups/probs) |
...
| 4 | Baseline profit agent | `agents/baseline_agent.ts` |
| 5 | Risk engine | `risk/rules.ts` |
| 6 | Daily runner automation | `runner/daily.ts` + scheduler config |
| 7 | Web dashboard MVP | `/web` Next.js pages for runs/decisions/performance |
| 8 | Research agent + evidence collector | `agents/research_agent.ts`, `evidence_collector.ts` |
| 9 | Learning/reflection loop | `learning/reflection.ts` + `strategy_state` table |
| 10 | Generic connector interface | `connectors/base_connector.ts` |
...
- Manifold API key/credentials for authenticated endpoints (Steps 2+).
...
## Next Actions
1. Implement Step 0: scaffold `core/agent_interface.ts` with types and decision-packet structure.
